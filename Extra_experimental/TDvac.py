...
class Actor():
    ...
    def accumulate_gradient(self):
        ...
        with tf.GradientTape() as tape:
            ...
            #Calculate loss
            loss_value = self.custum_loss(V_memory,actions, logits)
...
class Critic():
    ...
    def build_polic_network(self):
        ...
        # no hidden layer
        if(self.h1_dims == 0 and self.h2_dims==0):
            V_value = Dense(1, activation='linear')(input)
        #One hidden layer
        elif(self.h1_dims != 0 and self.h2_dims == 0):
            dense1 = Dense(self.h1_dims,activation='relu',kernel_regularizer=l2(0.01))(input)
            V_value = Dense(1, activation='linear')(dense1)
        #Two hidden layers
        else:
            dense1 = Dense(self.h1_dims,activation='relu',kernel_regularizer=l2(0.01))(input)
            dense2 = Dense(self.h2_dims, activation='relu',kernel_regularizer=l2(0.01))(dense1)
            V_value = Dense(1, activation='linear')(dense2)


        critic = Model(inputs = [input], outputs = [V_value])
        critic.summary()
    ...
    def learn(self, reward, next_state,next_action, V, done):
        # weights = self.critic.get_weights()

        #Get gradient of V function
        with tf.GradientTape() as tape:
            Vvalues = self.critic(next_state)
            tvs = self.critic.trainable_variables
            next_V = Vvalues[0]
        #Calculating Gradient on Q of current state and action with respect to weights(bias included) of the network
        grads = tape.gradient(next_V, tvs)

        V_ = np.array(next_V)
        # print(Q,Q_)
        #When done is true no need to take value of next state and change only the target value of present action
        TD_error = reward + self.gamma * V_ * (1 - int(done)) - V
