...
import numpy as np 
from konda import Actor, Critic
import pandas as pd
import os
...
if __name__ == '__main__':
    ...
    critic = Critic(lambda_ = Lambda, ALPHA = alpha2, input_dims=critic_input*n_actions, 
    ...
    + str(Lambda)+"_"+ str(gamma)+"_"+str(actor_batch_size)
    ...
    for i in (range(n_episodes)):
        ...
        while not done:
            ...
            probs = actor.actor.predict(state)[0]
            ...
            Qpi = critic.predict(actor_phi,probs,action)
            #Store all the parameters for later use
            actor.store_transition(observation, action, [Qpi])
            #Update weights
            critic.learn(reward, state_, action_, Qpi, done, actor, i,actor_batch_size)
