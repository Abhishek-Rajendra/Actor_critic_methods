'''
Here we are implementing Q-Actor Critic Algorithm
   ...
   #Create a path to save the output
    ...
    for i in range(n_episodes):
        ...
        while not done:
            ...
            Q_ = np.array([critic.critic.predict(state_)[0][action_]])

            actions = np.zeros([1, n_actions])
            actions[np.arange(1), action] = 1.0

            #Train Actor network
            actor.actor.fit([state, Q_pi], actions,verbose=0)

            #When done is true no need to take value of next state
            target[0,action] = reward + gamma * Q_ * (1 - int(done))
