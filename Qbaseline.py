...
class Actor():
    ...
    def learn(self, G, episode_num):
        
        episode_num +=1
        #Getting average Total Reward of an episode
        self.baseline = (self.baseline*(episode_num-1) + G)/episode_num
        ...
        Q_memory = Q_memory - self.baseline
...
class Critic():
    ...
    def build_polic_network(self):
        ...
        # no hidden layer
        if(self.h1_dims == 0 and self.h2_dims==0):
            Qvalue = Dense(self.n_actions, activation='linear')(input)
        #One hidden layer
        elif(self.h1_dims != 0 and self.h2_dims == 0):
    ...
    def learn(self):
        ...
        # return cost
        #For using it has baseline for actor
        return G[0]
