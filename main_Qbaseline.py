'''
This implementation is little different from Q actor-critic.
Here we update the Q network in TD(1) fasion after the
episode is completed. i.e G_t as the target function.
And while update actor we add a baseline that is the average total reward 
of all the episodes till then.
...
import numpy as np 
from Qbaseline import Actor, Critic
import pandas as pd
import os
   ...
   #Create a path to save the output
    ...
    for i in range(n_episodes):
        ...
        while not done:
            ...
            # critic.critic.fit(state, target, verbose=0)
        ...
        #Updating critic network and getting the total discounted reward for the episode
        G = critic.learn()
